...
# https://web.archive.org/web/20220824134917/https://cancerdatascience.org/blog/posts/pearson-correlation/
def pearson_corr(x, y):
    # Recast to np.float64 for precision errors:
    # small values were rounded to 0 causing divide by zero warnings
    x = x.astype(np.float64)
    y = y.astype(np.float64)

    xv = x - x.mean(axis=0)
    yv = y - y.mean(axis=0)
    xvss = np.sqrt(np.square(xv).sum(axis=0))
    yvss = np.sqrt(np.square(yv).sum(axis=0))
    num = xv.T @ yv
    denom = np.outer(xvss, yvss)
    result = num / denom

    # Recast to np.float32
    return np.maximum(np.minimum(result, 1.0), -1.0).astype(np.float32)


def singlesubject_seed_conn(seed_path, tc_path, mask="", transform=""):
    """Create a seed connectivity map for a single subject 4d timecourse.

    Args:
        seed_path str: Path to seed NIfTI
        tc_path str: Path to timecourse NIfTI
        mask: Path to brain space mask
        transform: Transform to apply to values. Valid values are "fz" and "zscore". "fz" is the
            Fischer Z (arctanh) transform, "zscore" is the z-standardized Fischer Z transformed values.
            Defaults to "" (no transform).

    Returns:
        Niimg-like: Seed connectivity map

    """
    if transform not in ["", "fz", "zscore"]:
        raise ValueError("Not valid transform method")
    if mask == "":
        mask = ds.get_img("MNI152_T1_2mm_brain_mask_dil")
    masker = maskers.NiftiMasker(mask).fit()
    tc = masker.transform(tc_path)
    seed_vec = masker.transform(seed_path)
    avg_signal = extract_avg_signal(tc, seed_vec)
    res = pearson_corr(avg_signal[:, np.newaxis], tc)
    if transform != "":
        res = np.arctanh(res)
    if transform == "zscore":
        res = stats.zscore(res, axis=None)

    res_img = masker.inverse_transform(res)
    return res_img
